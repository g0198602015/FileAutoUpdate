...
import com.example.fileautoupdate.db.ThreadDAO;
import com.example.fileautoupdate.db.ThreadDAOImpl;
import com.example.fileautoupdate.entities.FileInfo;
import com.example.fileautoupdate.entities.ThreadInfo;
...
//						"bytes=" + start + "-" + mThreadInfo.getEnd());
				...
				if (responseCode == 206 || responseCode == 200) //HttpStatus.SC_PARTIAL_CONTENT
				{
					...
					Log.i(AutoStartReceiver.TAG, "(DownloadTask) Error: the "+mFileInfo.getFileName()+" http response code is "+ responseCode);
	...
	private synchronized void checkAllThreadFinished()
	{
		...
		if (allFinished)
		{
			...
			AutoStartReceiver.updateVersion(mFileInfo.getFileName());
